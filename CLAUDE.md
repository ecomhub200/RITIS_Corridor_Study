...
| `data/Traffic_Signal.csv` | **Optional** signal inventory powering the Map tab's traffic-signal layer (202 Henrico signals). Header: `INO,Signal Name,Street 1,Street 2,FYA,Type,Latitude,Longitude,…,Coordination Group,Coordination Corridor,Dual Left`. Parsed by `parseSignals()`; rendered as uniform markers with two dropdowns — **Corridor** (coordination corridor) filters + zooms, then **Signal** pinpoints one. Absent file = layer simply hidden. | Committed by hand. |
