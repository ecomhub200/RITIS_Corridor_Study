...
| `data/Traffic_Signal.csv` | **Optional** signal inventory powering the Map tab's traffic-signal layer (202 Henrico signals). Header: `INO,Signal Name,Street 1,Street 2,FYA,Type,Latitude,Longitude,…,Coordination Group,Coordination Corridor,Dual Left`. Parsed by `parseSignals()`; markers can be colored/filtered by coordination corridor, type, detection camera, features, or pedestrian phase. Absent file = layer simply hidden. | Committed by hand. |
